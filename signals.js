...
function Finding({ num, kicker, children, cite, onDismiss, note, onNoteChange }) {
      ...
      {onNoteChange && (
        showNote ? (
          <div className="finding__note">
            <div className="finding__note-label">Your note</div>
            <textarea
              value={draft}
          ...
          <button className="finding__note-add" onClick={() => setShowNote(true)}>+ Add note</button>
        )
      )}
    </div>
  );
...
function HeroSummary({ title, rows, onDismiss, emptyText }) {
  return (
    <div className="hero-summary">
      ...
      <div className="hero-summary__rows">
        ...
        {rows.map((r, i) => (
          <div key={r.id || i} className={'hero-summary__row hero-summary__row--' + (r.tone || 'neutral')}>
