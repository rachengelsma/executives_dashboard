...
function Masthead({ chapter, onNav, badges = {}, onCmdK, layout = 'bauhaus', onLayoutChange, theme = 'light', onThemeChange, user, onUserChange, onExport, onShare }) {
  ...
  const [sidebarHidden, setSidebarHidden] = React.useState(() => {
    try { return localStorage.getItem('prts.sidebarHidden') === '1'; } catch (e) { return false; }
  });
  ...
  const LAYOUTS = [
    ...
    { id: 'editorial-compact', label: 'Editorial Compact', note: 'Serif typography, tighter density' },
    { id: 'compact',           label: 'Compact',           note: 'Dense for analysis' },
    { id: 'bauhaus',           label: 'Bauhaus',           note: 'Structural & color-blocked' },
    { id: 'brand',             label: 'Brand',             note: 'Matches the new website' },
  ];
